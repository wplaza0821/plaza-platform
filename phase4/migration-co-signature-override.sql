...
--      trigger (not just the UI), so PostgREST or raw SQL cannot bypass it either.
--   2. NEVER SILENT. An override records who, when, and a mandatory written reason.
--      Missing/blank reason => rejected.
--   3. NOT STICKY. Replacing the CO document clears the override, so a swapped-in
--      document is re-verified on its own merits.
...
-- 2. Single gate function for INSERT and UPDATE. Base gate + all-blocks gate,
--    with an owner-only override branch ahead of them. OLD is only dereferenced
--    on UPDATE (TG_OP guards), so one function serves both operations.
...
declare
  ...
  was_override   boolean := case when TG_OP = 'UPDATE'
                                 then coalesce(old.signature_override, false)
                                 else false end;
  old_file       text    := case when TG_OP = 'UPDATE' then old.file_path else null end;
begin
  ...
  if coalesce(new.signature_override, false) is true and was_override is not true then
  ...
  if was_override is true and coalesce(new.signature_override, false) is not true then
    if jwt_role <> 'owner' then
  ...
  -- A replacement document invalidates any prior override: re-verify on merit.
  if TG_OP = 'UPDATE'
     and new.file_path is distinct from old_file
     and coalesce(new.signature_override, false) is true
     and was_override is true then
  ...
  if new.status = 'approved' then
    ...
    --    must be signed. A blank block blocks approval regardless of role.
...
--     where tgrelid='change_orders'::regclass and not tgisinternal;
--   select prosrc like '%signature_override%' as has_override
--     from pg_proc where proname='plz_co_require_signature';
--
-- ROLLBACK (manual):
--   re-apply phase4/migration-co-signature-allblocks.sql to restore the pre-override
--   gate, and optionally:
